...
(function ($) {
    ...
    $.showDialog = function (options) {
        ...
        temp.URL = opt.URL;
        temp.OnLoad = opt.OnLoad;
        temp.InnerHtml = opt.InnerHtml;
        temp.InvokeElementId = opt.InvokeElementId;
        temp.Top = opt.Top;
        temp.Left = opt.Left;
        temp.Title = opt.Title;
        temp.OKEvent = opt.OKEvent;
        temp.CancelEvent = opt.CancelEvent;
        temp.ShowButtonRow = opt.ShowButtonRow;
        temp.MessageIcon = opt.MessageIcon;
        temp.MessageTitle = opt.MessageTitle;
        temp.Message = opt.Message;
        temp.ShowMessageRow = opt.ShowMessageRow;
        temp.Modal = opt.Modal;
        temp.Drag = opt.Drag;
        temp.AutoClose = opt.AutoClose;
        temp.ShowCloseButton = opt.ShowCloseButton;
        temp.Animator = opt.Animator;
        /****以下属性以小写开始，不要自行改变****/
        temp.dialogDiv = opt.dialogDiv;
        temp.bgDiv = opt.bgDiv;
        temp.parentWindow = opt.parentWindow;
        temp.innerFrame = opt.innerFrame;
        temp.innerWin = opt.innerWin;
        temp.innerDoc = opt.innerDoc;
        temp.zindex = opt.zindex;
        temp.cancelButton = opt.cancelButton;
        temp.okButton = opt.okButton;

        return temp;
    };
    $.alert=function(msg, func,left,top,modal, w, h)
    {
    	Dialog.alert(msg, func,left,top,modal, w, h);
    }
    $.confirm=function(msg,funcOK,funcCal,left,top,w,h,okButtonText,cancelButtonText)
    {
    	Dialog.confirm(msg, funcOK, funcCal,left,top, w, h,okButtonText,cancelButtonText);
    }
    
    var IMAGESPATH = 'static/zDialog/skins/orange/'; //图片路径配置
...
'
    ...
    Dialog.alert = function (msg, func,left,top,modal, w, h) {
        var w = w || 300,
            h = h || 110;
        var diag = new Dialog({
            Width: w,
        ...
        diag.InnerHtml = '<div style="background:#fff;height:'+h+'px"><table  style="background-color:#fff;" height="100%" border="0" align="center" cellpadding="10" cellspacing="0">\
    ...
    Dialog.confirm = function (msg, funcOK, funcCal,left,top, w, h,okButtonText,cancelButtonText) {
        var w = w || 300,
            h = h || 110;
        var diag = new Dialog({
            Width: w,
        ...
        diag.InnerHtml = '<div style="background:#fff;height:'+h+'px"><table height="100%" border="0" align="center" cellpadding="10" cellspacing="0">\
        ...
        debugger;
        if(okButtonText){
        	diag.okButton.value=okButtonText;
        };
        if(cancelButtonText){
        	diag.cancelButton.value=cancelButtonText;
        };
        diag.okButton.parentNode.style.textAlign = "center";
        diag.okButton.focus();
    };
