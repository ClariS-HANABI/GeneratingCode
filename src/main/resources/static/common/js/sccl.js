...
function addIframe(cur){
	...
	if (!isHas) {
		var tab = "<a href='javascript:;' class='content-tab active' style='cursor:pointer;' data-id='"+h+"'>"+ label +" <i class='icon-font'>&#xe617;</i></a>";
...
function closePage() {
	...
	if ($(this).parents(".content-tab").hasClass("active")) {
		...
		if ($(this).parents(".content-tab").prev(".content-tab").size()) {
            ...
            var prev_url = $(this).parents(".content-tab").data("id");
            $(this).parents(".content-tab").addClass("active");
            $(".body-iframe").each(function() {
                if ($(this).data("id") == prev_url) {
                    $(this).show().siblings(".body-iframe").hide();
                    return false
...
$(function(){
	...
	$.ajax({
        type:"POST",
        url:"roleMenu/selectMenuByRoleId",
        data:{
		...
		success:function(result){
			...
			var menu = [];
			var rootNode  ={};
			var rolemenu = result.roleMenuList;
			rootNode.id ='1';
			rootNode.name ='主菜单';
			rootNode.parentId ='0';
			rootNode.url ='';
			...
			rootNode.isHeader ='1';
			rootNode.childMenus=[];
			getMenus(rolemenu,'主菜单',rootNode);
			menus.push(rootNode);
			initMenu(menus,$(".side-menu"));
		}
	});
	...
	/*获取菜单icon随机色*/
	//getMathColor();
	function getMenus(rolemenu, parentMenuName, parentNode){
		for(var i =0;i<rolemenu.length;i++){
			var menu = rolemenu[i];
			if(menu.MENU_SUPERIOR ==parentMenuName){
				var secondNode  = {};
				secondNode.id =menu.MENU_ID;
				secondNode.name =menu.MENU_NAME;
				secondNode.parentId =parentNode.id;
				if(menu.MENU_URL == null){
					secondNode.url ="";
				}else{
					secondNode.url =menu.MENU_URL;
				}
				secondNode.icon ='';
				secondNode.order ='1';
				secondNode.isHeader ='0';
				secondNode.childMenus=[];
				getMenus(rolemenu, menu.MENU_NAME, secondNode);
				parentNode.childMenus.push(secondNode);
			}
		}
